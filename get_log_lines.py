...
class FileLocation:
    ...
    def __init__(self, cov_date=None, lib_code=None, path=r"C:\Users\megrust\Desktop\Workstuff\OCLC Datasync"):
        self.cov_date = '20250519'
        self.lib_code = 'e8w'
        #change these to what they are "subject"
        self.path = path

    @property
    def pathname(self):
        return(
            f'{self.path}\\Xrefs-{self.cov_date}-{self.lib_code}'
            ) 
    ...
    def mrc_file_name(self):
        ...
        return self.get_file_name('.mrc')

    
    @property
    def keyfilename(self):
        return self.get_file_name('keys.txt')
    ...
    def get_file_name(self, fragment):
        for (root,dirs,files) in os.walk(f'{self.pathname}'): 
            for file_name in files:
